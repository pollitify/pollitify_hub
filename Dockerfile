...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential git node-gyp pkg-config python-is-python3 postgresql-client postgresql-contrib libpq-dev imagemagick  libmagickwand-dev sudo && \
    rm -rf /var/lib/apt/lists /var/cache/apt/archives
