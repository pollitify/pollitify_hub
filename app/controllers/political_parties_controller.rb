class PoliticalPartiesController < ApplicationController
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to political_parties_path, status: :see_other, notice: "Political party was successfully destroyed." }
  ...
  private
    ...
    def set_political_party
      @political_party = PoliticalParty.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def political_party_params
      params.expect(political_party: [ :name, :fid, :url ])
    end
end
